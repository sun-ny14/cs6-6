(function(){
    ...
    function recordSummary(date){
        const value=dayValue(date),parts=[],lessons=Object.values(value.periodNotes||{}).filter(item=>String(item?.note||'').trim()).length+(value.lessonNote?1:0);
        if(state.filter==='all'||state.filter==='counsel')if((value.counseling||[]).length)parts.push(`상담 ${(value.counseling||[]).length}`);
        if(state.filter==='all'||state.filter==='lesson')if(lessons)parts.push(`수업 ${lessons}`);
        if(state.filter==='all'||state.filter==='schedule')if((value.schedules||[]).length)parts.push(`일정 ${(value.schedules||[]).length}`);
        if(state.filter==='all'&&noticeFor(date).text.trim())parts.push('공지');return parts;
    }
    ...
    function renderModal(date,mode=state.entryMode||'all'){
        ...
        const noticeSection=`<section class="journal-editor-section" data-entry-section="all"><div class="journal-section-head"><h3>📢 공지</h3></div><textarea id="journal-notice-text" rows="4" placeholder="엔터로 공지 항목을 나누세요.">${esc(notice.text)}</textarea><div id="journal-notice-checks" class="journal-notice-check"></div></section>`;
        const scheduleSection=`<section class="journal-editor-section" data-entry-section="schedule"><div class="journal-section-head"><h3>📅 교사 업무 일정</h3><button class="btn btn--sm" data-add-work type="button">+ 일정</button></div><div class="journal-rows">${(value.schedules||[]).map(workRow).join('')||'<div class="journal-empty">등록된 업무 일정이 없습니다.</div>'}</div></section>`;
        const counselSection=`<section class="journal-editor-section" data-entry-section="counsel"><div class="journal-section-head"><h3>💬 상담 기록</h3><button class="btn btn--sm" data-add-counsel type="button">+ 상담</button></div><div class="journal-rows">${(value.counseling||[]).map(counselingRow).join('')||'<div class="journal-empty">등록된 상담 기록이 없습니다.</div>'}</div></section>`;
        const lessonSection=`<section class="journal-editor-section" data-entry-section="lesson"><div class="journal-section-head"><h3>📖 교시별 수업일지</h3><span class="journal-status">시간표 과목 자동 반영</span></div><div class="journal-periods">${PERIODS.map(period=>`<div class="journal-period" data-period="${period}" data-subject="${esc(periods[period].subject)}"><strong>${period} · <span>${esc(periods[period].subject||'과목 미등록')}</span></strong><textarea rows="3" data-period-note placeholder="${period} 수업 기록">${esc(periods[period].note)}</textarea></div>`).join('')}</div></section><section class="journal-editor-section" data-entry-section="lesson"><div class="journal-section-head"><h3>➕ 추가 기록</h3></div><textarea id="journal-extra-note" rows="8" placeholder="생활지도, 학급 운영, 다음 날 준비 등 교시 외 기록을 작성하세요.">${esc(value.lessonNote||'')}</textarea></section>`;
        ...
        if(mode!=='all')modal.querySelectorAll('[data-entry-section]').forEach(section=>{if(section.dataset.entrySection!==mode)section.remove();});
