...
function startApp(){
   ...
   const canManage=
    ...
    db.ref(admin?'settings':'publicSettings').on('value',snap=>{
        ...
        // 학생용 publicSettings에는 역할 정보가 없으므로 별도 cleaningSettings 리스너를 유지한다.
        if(admin){
...
if (typeof refreshCheckinGuide === 'function') {
    ...
    if(!admin){
        db.ref('cleaningSettings').on('value',snap=>{
            ...
            // 미러링이 아직 준비되지 않았으면 로그인 세션의 본인 배정을 보존한다.
            window.studentRoles=cleaning.studentRoles||window.studentRoles||{};
            window.cleaningAssignments=cleaning.cleaningAssignments||window.cleaningAssignments||{};
