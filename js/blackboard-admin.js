...
(function () {
    ...
    function ensureStyles() {
        ...
        style.textContent = `
            ...
            #bb-schedule-form-container .bb-notice-calendar{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:6px;margin:16px 0}.bb-calendar-toolbar{margin-top:16px}.bb-calendar-toolbar strong{min-width:140px;text-align:center;font-size:23px}.bb-calendar-weekday{text-align:center;padding:8px 0;font-size:16px;font-weight:900;color:#5b6576}.bb-calendar-weekday:first-child{color:#c0392b}.bb-calendar-weekday:nth-child(7){color:#2472aa}
    ...
    function renderNoticeStrip() {
        ...
        const headings = ['일','월','화','수','목','금','토'].map(day => `<div class="bb-calendar-weekday">${day}</div>`).join('');
        calendar.innerHTML = headings + Array.from({length:count}, (_, index) => {
            const day = index - leading + 1;
            if (day < 1 || day > days) return '<div class="bb-calendar-blank" aria-hidden="true"></div>';
            const date = addDays(first, day - 1);
