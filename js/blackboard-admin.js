...
(function () {
    ...
    function ensureStyles() {
        ...
        style.textContent = `
            .bb-admin-grid{display:grid;gap:20px}.bb-panel{padding:22px;border:1px solid var(--ui-line);border-radius:16px;background:var(--ui-surface);font-family:var(--ui-font-display)}.bb-panel h3{margin:0 0 16px;color:var(--ui-text)}.bb-toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.bb-toolbar h3{margin:0}.bb-toolbar--between{justify-content:space-between}.bb-toolbar--gap{margin-top:16px}.bb-toolbar--gap-sm{margin-top:12px}.bb-panel--notice{background:var(--ui-gold-soft);border-color:var(--ui-gold-line)}.bb-btn{padding:11px 16px;border:0;border-radius:9px;font-weight:900;cursor:pointer}.bb-btn.primary{background:var(--ui-accent);color:var(--ui-on-accent)}.bb-btn.green{background:var(--ui-good-soft);color:var(--ui-good);box-shadow:inset 0 0 0 1px var(--ui-good-line)}.bb-btn.dark{background:var(--ui-surface);color:var(--ui-accent-deep);box-shadow:inset 0 0 0 1px var(--ui-line-strong)}.bb-btn.light{background:var(--ui-surface-soft);color:var(--ui-accent-deep)}.bb-btn.red{background:transparent;color:var(--ui-bad);box-shadow:inset 0 0 0 1px var(--ui-bad-line)}.bb-btn.red:hover{background:var(--ui-bad-soft)}.bb-date-strip,.bb-day-tabs{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0}.bb-date-btn,.bb-day-btn{position:relative;padding:10px 13px;border:2px solid var(--ui-line);border-radius:10px;background:var(--ui-surface);font-weight:850;cursor:pointer}.bb-date-btn.active,.bb-day-btn.active{border-color:var(--ui-accent);background:var(--ui-accent-soft);color:var(--ui-accent-deep)}.bb-day-btn.is-today::after{content:"";position:absolute;top:5px;right:5px;width:7px;height:7px;border-radius:50%;background:var(--ui-gold)}.bb-editor{display:grid;gap:10px;margin-top:14px}.bb-period-row{display:grid;grid-template-columns:95px minmax(130px,1fr) auto;gap:10px;align-items:center;padding:12px;border:1px solid var(--ui-line);border-radius:12px;background:var(--ui-surface-soft)}.bb-period-row input[type=text],.bb-period-row textarea,.bb-panel input[type=date],.bb-panel textarea{padding:10px;border:1px solid var(--ui-line-strong);border-radius:8px;font:inherit;box-sizing:border-box}.bb-period-extra{grid-column:2/-1;display:grid;grid-template-columns:1fr;gap:9px}.bb-period-extra textarea{width:100%;resize:vertical}.bb-check{display:flex;align-items:center;gap:6px;font-weight:850;white-space:nowrap}.bb-modal{position:fixed;inset:0;z-index:1000000;display:grid;place-items:center;padding:18px;background:rgba(15,23,42,.62)}.bb-modal[hidden]{display:none}.bb-modal-dialog{width:min(1100px,96vw);max-height:94vh;overflow:auto;padding:25px;border-radius:20px;background:var(--ui-surface);box-shadow:var(--ui-shadow)}.bb-modal-head{display:flex;justify-content:space-between;align-items:center;gap:12px}.bb-modal-head h2{margin:0}.bb-time-grid{display:grid;grid-template-columns:repeat(2,minmax(320px,1fr));gap:12px;margin:18px 0 24px}.bb-time-row{padding:14px;border-radius:10px;background:var(--ui-surface-soft)}.bb-time-row strong{display:block;margin-bottom:8px}.bb-time-row div{display:grid;grid-template-columns:minmax(145px,1fr) auto minmax(145px,1fr);align-items:center;gap:8px}.bb-time-row input[type=time]{width:100%;min-width:0;padding:10px 12px;box-sizing:border-box;font:inherit}.bb-notebook-grid{display:grid;grid-template-columns:repeat(5,minmax(180px,1fr));gap:12px;margin-top:20px}.bb-notebook-day{padding:14px;border-radius:14px;background:var(--ui-surface-soft);border:1px solid var(--ui-line)}.bb-notebook-day h3{margin:0 0 12px;color:var(--ui-text)}.bb-notebook-item{padding:12px;margin-top:9px;border-radius:10px;background:var(--ui-surface);border-left:5px solid var(--ui-accent)}.bb-notebook-item strong,.bb-notebook-item span{display:block}.bb-notebook-item p{margin:7px 0 0;white-space:pre-wrap;line-height:1.45}.bb-status{color:var(--ui-muted);font-weight:800}.bb-empty{padding:28px;border:1px dashed var(--ui-line-strong);border-radius:12px;text-align:center;color:var(--ui-muted);font-weight:800}.bb-empty--error{color:var(--ui-bad)}
            .bb-assignment-form{display:grid;grid-template-columns:1.2fr 1fr auto auto;gap:10px;align-items:center}.bb-assignment-form input{padding:11px;border:1px solid var(--ui-line-strong);border-radius:8px}.bb-assignment-form textarea{grid-column:1/-1}.bb-required-check{display:flex;gap:7px;align-items:center;padding:10px;border-radius:8px;background:var(--ui-warn-soft);color:var(--ui-bad);font-weight:900;white-space:nowrap}.bb-assignment-list{display:grid;gap:10px;margin-top:16px}.bb-assignment-row{display:grid;grid-template-columns:1fr auto;gap:12px;padding:15px;border:1px solid var(--ui-line);border-radius:12px;background:var(--ui-surface-soft)}.bb-assignment-row strong,.bb-assignment-row span{display:block}.bb-assignment-row p{margin:8px 0 0;white-space:pre-wrap}.bb-student-checks{display:flex;gap:7px;flex-wrap:wrap;margin-top:12px}.bb-student-check{min-width:58px;padding:9px;border:2px solid var(--ui-gold-line);border-radius:9px;background:var(--ui-gold-soft);color:var(--ui-gold);font-weight:900;cursor:pointer}.bb-student-check.is-done{border-color:var(--ui-good);background:var(--ui-good);color:var(--ui-on-accent);text-decoration:line-through}
            #bb-save-week,#bb-save-settings{width:100%;padding:16px}#bb-save-week{margin-top:16px}#bb-save-settings{margin-top:18px}#bb-settings-content>h3:first-of-type{margin-top:24px}
    ...
    function renderWeekTabs() {
        ...
        tabs.innerHTML = WEEKDAYS.map((day,index) => {
            ...
            return `<button type="button" class="bb-day-btn${date===state.selectedWeekDate?' active':''}${date===today?' is-today':''}" data-week-date="${date}">${day.label} ${date.slice(5)}${state.week[date]?' ✓':''}</button>`;
        }).join('');
    ...
    function renderMain() {
        ...
        container.innerHTML = `<div class="bb-admin-grid">
            <section class="bb-panel bb-panel--notice">
                <h3>📅 공지 달력</h3>
            ...
            <section class="bb-panel">
                <div class="bb-toolbar bb-toolbar--between"><h3>🗓️ 주간 시간표</h3><button class="bb-btn dark" id="bb-open-settings">⚙️ 교시 시간·기본 시간표 설정</button></div>
                ...
                <button class="bb-btn primary" id="bb-save-week">💾 이 주 시간표 저장</button>
            </section>
        </div>`;
    ...
    async function deleteNotice() {
        ...
        try {
            ...
            if (typeof window.showUndoBar === 'function' && before != null) {
                window.showUndoBar(`${date} 공지를 삭제했습니다.`, async () => {
                    await db.ref(`blackboard/notices/${date}`).set(before);
                    state.notices[date] = before;
                    renderNoticeStrip();
                }, 12);
            } else {
    ...
    async function openWeeklyNotebook() {
        ...
        }catch(error){console.error('주간 배움공책 & 메모 조회 실패:',error);content.innerHTML='<div class="bb-empty bb-empty--error">주간 배움공책 &amp; 메모를 불러오지 못했습니다.</div>';}
    }
    ...
    function openSettings() {
        ...
        content.innerHTML=`<h3>⏰ 교시 시간 설정</h3><div class="bb-time-grid">${PERIODS.map(name=>{const time=timeFor(name);return `<div class="bb-time-row"><strong>${name}</strong><div><input type="time" class="bb-time-start" data-period="${name}" value="${time.startTime}"><span>~</span><input type="time" class="bb-time-end" data-period="${name}" value="${time.endTime}"></div></div>`;}).join('')}</div>
            <h3>📚 월~금 기본 시간표</h3><p class="bb-status">이동수업은 여기에서 체크하면 매주 그대로 복사됩니다.</p><div class="bb-day-tabs">${WEEKDAYS.map(day=>`<button class="bb-day-btn${day.key===state.selectedBaseDay?' active':''}" data-base-day="${day.key}">${day.label}요일</button>`).join('')}</div><div class="bb-editor" id="bb-base-editor"></div><button class="bb-btn primary" id="bb-save-settings">기본 설정 저장</button>`;
    ...
    window.initBlackboardAdmin=async function () {
        ...
        const today=todayKst();state.selectedNoticeDate=today;state.selectedWeekStart=mondayOf(today);
        const todayWeekday=new Date(`${today}T00:00:00Z`).getUTCDay();
        // 주말엔 평일 탭이 없으니 월요일로, 평일엔 오늘 요일 탭이 기본으로 열리게 한다.
        state.selectedWeekDate=(todayWeekday>=1&&todayWeekday<=5)?today:state.selectedWeekStart;
        try{
            ...
            renderMain();
        }catch(error){console.error('전자칠판 관리 로딩 실패:',error);container.innerHTML='<div class="bb-empty bb-empty--error">전자칠판 정보를 불러오지 못했습니다.</div>';}
    };
