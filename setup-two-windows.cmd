...
powershell.exe -NoLogo -NoProfile -NonInteractive -Command "$ErrorActionPreference='Stop'; $root=$env:CS6_SETUP_ROOT; $file=Join-Path $root '.vscode\settings.json'; $launcher=Join-Path $root '.vscode\open-two-views.cmd'; if (!(Test-Path -LiteralPath $launcher)) { throw 'Extract all ZIP files into the existing project folder first.' }; $settings=Get-Content -LiteralPath $file -Raw | ConvertFrom-Json; Copy-Item -LiteralPath $file -Destination ($file+'.two-windows-backup') -Force; $updates=@{'liveServer.settings.host'='127.0.0.1'; 'liveServer.settings.port'=5500; 'liveServer.settings.root'='/'; 'liveServer.settings.AdvanceCustomBrowserCmdLine'=([string][char]34+$launcher+[char]34); 'liveServer.settings.CustomBrowser'=$null; 'liveServer.settings.NoBrowser'=$false}; foreach ($key in $updates.Keys) { $settings | Add-Member -MemberType NoteProperty -Name $key -Value $updates[$key] -Force }; [IO.File]::WriteAllText($file,($settings | ConvertTo-Json -Depth 50)+[Environment]::NewLine,(New-Object Text.UTF8Encoding($false))); Write-Host 'Two-window Go Live setup complete.'"
if errorlevel 1 (
